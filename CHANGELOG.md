...
### Added
- **全局日志系统**：引入 `logger` 插件并封装 `Log` 工具类，统一全项目的日志输出流，支持分级显示与 Emoji 标识，大幅提升调试效率。
- **AI 业务中台化**：新增 `AICoachService` 作为 AI 业务统一出口，整合饮食解析、动作深度解读、周报生成等核心 AI 逻辑。
- **饮食解析 RAG 优化**：在 AI 解析过程中引入召回机制，优先匹配《中国食物成分表》中的标准食物代码，提升解析精度。
- **LLM 熔断与故障转移**：重构 `LLMService` 接入 `ConfigService` 远程配置，实现多模型自动 failover 机制及 429 频率限制下的 6 小时熔断保护。
- **动作解读频率控制**：为 AI 动作解读功能新增 10 秒速率限制，优化资源利用并提升交互体验。

### Changed
- **饮食识别交互升级**：优化 AI 饮食识别反馈逻辑，在识别失败或 API 异常时自动保留用户的历史输入及前序识别结果，防止数据丢失。
- **版本更迭**：应用版本号正式升级至 v1.3.6。
- **服务鲁棒性提升**：重写了 `AppUpdateService` 与 `BackupService` 等核心服务的日志上报逻辑，并适配了 Android 端的多架构下载链接分配策略。
